...
uses Windows, Classes, SysUtils, Graphics, ActiveX, CommCtrl, GDIPAPI, toolu;
...
begin
  TopLeft:= Point(x, y);
  UpdateLayeredWindow(hWnd, 0, @topleft, nil, 0, nil, 0, nil, 0);
end;
//--------------------------------------------------------------------------------------------------
procedure LoadImage(imagefile: string; MaxSize: integer; default: boolean; var image: pointer; var srcwidth, srcheight: uint);
var
...
begin
  ...
  try
    ...
    srcheight := h;
    // downscale image //
    if (integer(w) > MaxSize) and (w > 128) and (MaxSize <= 192) then
    begin
      imgTemp := image;
...
begin
  result:= 'name=' + pchar(@fData.name) + ';' +
    ...
    'color2=$' + inttohex(fData.color2, 8) + ';' +
...
begin
  ...
  try font.color2:= StringToColor(FetchValue(str, 'color2=', ';'));
  except end;
...
begin
  ...
  result:= fadeto and $ff000000 + cardinal(r) shl 16 + cardinal(g) shl 8 + cardinal(b);
end;
//------------------------------------------------------------------------------
procedure RGBtoHLS(color: uint; out h, l, s: integer);
var
